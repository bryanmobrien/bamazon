    ...
    getProducts(`SELECT * FROM products`, con, function(res){
        ...
        askQuestions(function(answers){
           ...
           //if the id does not exist then give the user an error and end the database connection 
            ...
            getProducts(`UPDATE products SET stock_quantity=${ parseInt(item[0].stock_quantity) - parseInt(answers.quantity)},product_sales=${item[0].product_sales + totalPrice} WHERE id=${item[0].id}`, con, function(resolve){
